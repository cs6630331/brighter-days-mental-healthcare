	...
	<main>
		...
		<section class="doc-card-group">
			<?php
				...
				$stmt = $pdo->prepare("SELECT doctor_id, CONCAT(doctor_name, ' ', doctor_surname) AS doctor_fullname FROM `_doctor` WHERE is_active = 1");
				$stmt->execute();
			?>
			<?php while ($row = $stmt->fetch()): ?>
				<article class="doc-card">
					...
					<div>
						...
						<a class="button" class="button" href="/doctor.php?id=<?=$row["doctor_id"]?>">
